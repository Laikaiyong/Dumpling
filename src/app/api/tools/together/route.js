...
export async function POST(request) {
  try {
    ...
    if (intentClassification) {
      ...
      messages.push({
        role: "system",
        content: systemInstruction || `You are an intent detection system. Analyze the user's message and determine if they are asking about:
...
Always fallback to token_price when in doubt`
    ...
    const payload = {
          ...
          "function": {
            ...
            "parameters": {
              ...
              "properties": {
                "primaryIntent": {
                  "type": "string",
                  "enum": ["token_price", "wallet_analysis", "customer_support"],
      ...
      tool_choice: intentClassification ? { type: "function", function: { name: "detect_intent" } } : undefined
    };
    ...
    if (intentClassification && data.choices[0]?.message?.tool_calls?.[0]?.function) {
      // Parse the tool call result
      try {
        console.log(data.choices[0].message.tool_calls[0].function.arguments);
        const toolCallResult = JSON.parse(data.choices[0].message.tool_calls[0].function.arguments);
        result = {
